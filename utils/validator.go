...
func Verify(st interface{}, roleMap Rules) (err error) {
	...
	for i := 0; i < num; i++ {
		tagVal := typ.Field(i)
		val := val.Field(i)
		// 优先使用 json tag 作为校验 key
		key := tagVal.Name
		if jsonTag := tagVal.Tag.Get("json"); jsonTag != "" && jsonTag != "-" {
		...
		if len(roleMap[key]) > 0 {
			for _, v := range roleMap[key] {
				switch {
				case v == "notEmpty":
					if isBlank(val) {
						return errors.New(key + "值不能为空")
					}
				case strings.Split(v, "=")[0] == "regexp":
					if !regexpMatch(strings.Split(v, "=")[1], val.String()) {
						return errors.New(key + "格式校验不通过")
					}
				case compareMap[strings.Split(v, "=")[0]]:
					if !compareVerify(val, v) {
						return errors.New(key + "长度或值不在合法范围," + v)
					}
				}
			}
		}
